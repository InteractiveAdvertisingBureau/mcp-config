...
export function generateCRUDTools(schemas, storage) {
  ...
  for (const schemaKey of Object.keys(schemas)) {
    ...
    objectOperations[objType] = ['create', 'update', 'get', 'list', 'search'];
  }
...
function generateToolHandler(objType, operation, schema, storage, storageKey) {
  // Use plural form for list and search operations to match tool definitions
  const objectName = (operation === 'list' || operation === 'search') ? `${objType.toLowerCase()}s` : objType.toLowerCase();
  const toolName = `${operation}_${objectName}`;
...
function generateToolDefinition(objType, operation, schema) {
  // Use plural form for list and search operations to match handlers
  const objectName = (operation === 'list' || operation === 'search') ? `${objType.toLowerCase()}s` : objType.toLowerCase();
  const toolName = `${operation}_${objectName}`;
  const descriptions = {
