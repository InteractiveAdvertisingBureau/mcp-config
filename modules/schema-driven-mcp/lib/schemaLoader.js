...
import { getCustomSchema, isUsingCustomSchema } from './schemaStorage.js';
...
export function loadSchema(schemaPath = null) {
  ...
  if (customSchema) {
    console.log(`✅ Schema loaded: ${customSchema.name} v${customSchema.version} (Custom)`);
    return customSchema;
  }
  ...
  try {
    ...
    console.log(`✅ Schema loaded: ${schema.name} v${schema.version} (Default)`);
    return schema;
  } catch (error) {
...
export function loadManifest(manifestPath = null) {
  ...
  try {
    ...
    console.warn(`⚠️  Manifest file not found at ${path}, using default manifest`);
    // Return default manifest when file doesn't exist
    return {
      name: 'opendirect-mcp-server',
      version: '2.1.0',
      description: 'OpenDirect 2.1 MCP Server',
      capabilities: {
        tools: true,
        resources: true,
        prompts: false,
